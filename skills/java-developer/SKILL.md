...
Spring Boot development with DDD, rich domain models, layered architecture, and solid testing. The YAML `description` lists when to apply this skill; typical triggers include REST APIs, Spring Data JPA, Spring Security, validation, configuration, refactoring, and domain modeling.

## Scope

**In scope:** Spring Boot apps; REST APIs; Spring Data JPA; Spring Security; unit/integration/E2E testing; clean or layered architecture; DI; configuration and profiles; logging, metrics, and tracing; error handling and validation; performance; cloud-native patterns.

**Out of scope:** Frontends (React, Angular, etc.); DevOps/infrastructure (use devops-engineer); non-Spring Java stacks; Android; desktop apps.

**Technology stack (typical):** Java 25; Spring Boot 4.x; JUnit 5; Mockito 5.x; TestContainers for integration tests; use the project’s build tool (Maven and Gradle are common).
...
For detailed DDD and rich domain patterns (including domain vs JPA separation), see [patterns.md — Domain-Driven Design and rich domain patterns](references/patterns.md#domain-driven-design-and-rich-domain-patterns).
...
- **Quality review**: [quality-review.md](references/quality-review.md) — Readability, naming, Java 25/Spring Boot 4.x idioms, severity tags
- **Security review**: [security-review.md](references/security-review.md) — Injection risks, input validation, sensitive data exposure, severity tags
- **Clean code and anti-patterns**: [patterns.md](references/patterns.md) — Naming, method design, anemic domain, god service, transaction anti-patterns, checklist, DDD
- **General test strategy** (cross-stack): [test-engineer/SKILL.md](../test-engineer/SKILL.md) — planning, coverage mindset, review-style checklists
