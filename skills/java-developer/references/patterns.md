...
## Domain-Driven Design and rich domain patterns

Keep **domain types** free of JPA (`@Entity`, `@Table`, etc.). Persistence uses separate **entity classes** (e.g. `OrderEntity`) and mapping between domain and persistence in the infrastructure or application boundary—see [Anemic Domain Model](#anemic-domain-model) below for full examples.
...
```java
// ❌ Bad - Anemic domain type (plain data + procedural rules in the service)
// Domain Order has no JPA annotations; the problem is missing behavior on the type.
public class Order {
    private Long id;
...
// ✅ Good - Rich domain model (still no JPA on the domain type)
public class Order {
    private Long id;
...
// Persistence is a separate type (JPA only here—not on domain Order)
@Entity
@Table(name = "orders")
public class OrderEntity {
    ...
    // ... columns, JPA mappings; map to/from domain Order in repository or mapper
}
```
...
For DDD anti-patterns and rich domain guidance, see [Domain-Driven Design and rich domain patterns](#domain-driven-design-and-rich-domain-patterns) above.
...
- [ ] Mostly unit tests (typical target on the order of ~70%)
- [ ] Integration tests for boundaries (typical ~20%)
- [ ] Few end-to-end tests (typical ~10%)
